...
$BackupRoot = Join-Path $Root ".vendome-backups\a5-invoice-data-v10.3-$Stamp"
...
try {
    ...
    if (-not (Test-Path -LiteralPath $payloadInvoice)) {
        throw "A5 v10.3 invoice template payload is missing."
    }
...
'@
    ...
    # stay on stale list data. v10.3 isolates this exact function.
    # --------------------------------------------------------------
...
'@
    ...
    foreach ($token in @(
        ...
        if (-not $invoiceCheck.Contains($token)) {
            throw "A5 v10.3 invoice-template validation failed: $token"
        }
    }
    ...
    Write-Host ""
    Write-Host "v10.3 README/contract validations passed." -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "SUCCESS: VENDOME A5 invoice data mapping v10.3 installed." -ForegroundColor Green
    ...
    Write-Host "Rolling back every v10.3 invoice change..." -ForegroundColor Yellow
    Restore-All
