...
public partial class MainWindow : Window
{
    ...
    private void SetFullScreen(bool enabled)
    {
        ...
        else
        {
            ...
            Left = SystemParameters.WorkArea.Left +
                   Math.Max(0, (SystemParameters.WorkArea.Width - Width) / 2);
            Top = SystemParameters.WorkArea.Top +
                  Math.Max(0, (SystemParameters.WorkArea.Height - Height) / 2);
        }
    ...
    private async Task InitializeDashboardAsync()
    {
        ...
        try
        {
            ...
            {
                ShowStartupError(
                    "فایل‌های رابط React پیدا نشد.",
                    "داخل GoldInvoice.Client دستور npm ci و npm run build را اجرا کن، سپس Rebuild Solution بزن.");
                return;
            }
            ...
            if (DashboardWebView.CoreWebView2 is null)
            {
                ...
                CoreWebView2 initializedWebView = DashboardWebView.CoreWebView2 ??
            ...
            CoreWebView2 webView = DashboardWebView.CoreWebView2 ??
                throw new InvalidOperationException("WebView2 is not initialized.");
            webView.Navigate(
                $"https://{AppHostName}/index.html#/dashboard");
        }
        catch (WebView2RuntimeNotFoundException)
        {
            ShowStartupError(
                "Microsoft Edge WebView2 Runtime نصب نیست.",
                "WebView2 Runtime را نصب کن و برنامه را دوباره اجرا کن.");
        }
        catch (Exception)
        {
            ShowStartupError(
                "برنامه اجرا نشد.",
                "فایل‌های React، WebView2 Runtime و دسترسی پوشهٔ LocalAppData را بررسی کن.");
        }
        finally
        {
        ...
        CoreWebView2NavigationCompletedEventArgs e)
    {
        ...
        ShowStartupError(
            "بارگذاری رابط کامل نشد.",
            $"WebView2 نتوانست رابط محلی را بارگذاری کند: {e.WebErrorStatus}");
    }
        ...
        CoreWebView2WebMessageReceivedEventArgs e)
    {
        if (!Uri.TryCreate(e.Source, UriKind.Absolute, out Uri? source) ||
            !string.Equals(source.Host, AppHostName, StringComparison.OrdinalIgnoreCase))
        {
            return;
        }
        ...
        try
        {
            ...
            if (requestJson.Length > 512 * 1024)
            {
                throw new DesktopBridgeException(
                    "request_too_large",
                    "درخواست رابط بیش از حد بزرگ است.",
                    413);
            }
            ...
            JsonElement result = await HandleBridgeRequestAsync(
                requestType,
                payload,
        ...
        {
            PostBridgeResponse(new
            {
                id = requestId ?? string.Empty,
                ok = false,
                error = new { exception.Code, exception.Message, exception.Status }
            });
        }
        catch (JsonException)
        {
            PostBridgeResponse(new
            {
                ...
                error = new
                {
                    ...
                    message = "ساختار درخواست رابط معتبر نیست.",
                    status = 400
                }
            });
        }
        catch (Exception)
        {
            PostBridgeResponse(new
            {
                ...
                {
                    code = "desktop_error",
                    message = "برنامهٔ دسکتاپ نتوانست درخواست را کامل کند.",
                    status = 500
                }
            });
        }
    ...
    private async Task<JsonElement> HandleBridgeRequestAsync(
        string requestType,
        JsonElement payload,
        ...
        switch (requestType)
        {
            case "runtime.get":
                return JsonSerializer.SerializeToElement(
                    _settingsStore.GetRuntimeConfiguration(_secretStore.Exists),
                    JsonOptions);
            ...
            case "auth.login":
                return await _authBroker.LoginAsync(payload, cancellationToken);
            case "auth.refresh":
                return await _authBroker.RefreshAsync(cancellationToken);
            case "auth.mfa.setup":
                return await _authBroker.SetupMfaAsync(payload, cancellationToken);
            case "auth.mfa.enable":
                return await _authBroker.EnableMfaAsync(payload, cancellationToken);
            case "auth.logout":
                return await _authBroker.LogoutAsync(payload, cancellationToken);
            case "auth.clear":
                _authBroker.ClearSession();
            ...
            case "invoice.document":
            {
                ...
                if (action is not ("preview" or "save" or "print"))
                {
                    throw new DesktopBridgeException(
                        "invalid_request",
                        "عملیات فاکتور معتبر نیست.",
                        400);
                }
                ...
                CoreWebView2Environment environment = DashboardWebView.CoreWebView2?.Environment ??
                    throw new DesktopBridgeException(
                        "desktop_not_ready",
                        "میزبان دسکتاپ هنوز آماده نیست.",
                        409);
                var invoiceWindow = new InvoiceDocumentWindow(
                    this,
                    environment,
                    html,
                    suggestedFileName);
                ...
                if (action == "print")
                {
                    ...
                    return JsonSerializer.SerializeToElement(
                        new
                        {
                            ...
                            opened = true,
                            saved = false,
                            printed = printStatus == CoreWebView2PrintStatus.Succeeded,
                            printerName = "Windows default printer",
                            failureCode = printStatus switch
                            {
                                CoreWebView2PrintStatus.Succeeded => (string?)null,
                                CoreWebView2PrintStatus.PrinterUnavailable => "PRINTER_UNAVAILABLE",
                                _ => "PRINT_FAILED"
                            }
                        },
                        JsonOptions);
                ...
                if (action == "save")
                {
                    ...
                    return JsonSerializer.SerializeToElement(
                        new
                        {
                            action,
                            opened = true,
                            saved = savedPath is not null,
                            fileName = savedPath is null ? null : Path.GetFileName(savedPath)
                        },
                        JsonOptions);
                }

                return JsonSerializer.SerializeToElement(
                    new { action, opened = true, saved = false },
                    JsonOptions);
            }
            default:
                throw new DesktopBridgeException(
                    "unsupported_command",
                    "فرمان درخواستی توسط برنامهٔ دسکتاپ پشتیبانی نمی‌شود.",
                    400);
        }
    }
    ...
    {
        if (!Uri.TryCreate(DashboardWebView.CoreWebView2?.Source, UriKind.Absolute, out Uri? source) ||
            !string.Equals(source.Host, AppHostName, StringComparison.OrdinalIgnoreCase))
        {
            return;
        }
    ...
    private static string GetRequiredString(JsonElement element, string propertyName, int maximumLength)
    {
            ...
            string.IsNullOrWhiteSpace(property.GetString()))
        {
            throw new DesktopBridgeException(
                "invalid_request",
                "اطلاعات درخواست رابط کامل نیست.",
                400);
        }
        ...
        if (value.Length > maximumLength)
        {
            throw new DesktopBridgeException(
                "invalid_request",
                "یکی از مقادیر درخواست بیش از حد بلند است.",
                400);
        }
        ...
        int maximum)
    {
            ...
            value > maximum)
        {
            throw new DesktopBridgeException(
                "invalid_request",
                "یکی از مقادیر عددی درخواست معتبر نیست.",
                400);
        }
