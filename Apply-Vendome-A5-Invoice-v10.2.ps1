...
$BackupRoot = Join-Path $Root ".vendome-backups\a5-invoice-data-v10.2-$Stamp"
...
$Files = @($InvoiceTemplatePath, $OperationsPath, $TypesPath, $DesktopRendererPath)
...
try {
    ...
    if (-not (Test-Path -LiteralPath $payloadInvoice)) {
        throw "A5 v10.2 invoice template payload is missing."
    }
...
'@
    ...
    if ($invoicesStart -lt 0 -or $invoicesEnd -lt 0 -or $invoicesEnd -le $invoicesStart) {
        throw "Could not isolate InvoicesPage."
    }

    $invoicesBlock = $operations.Substring($invoicesStart, $invoicesEnd - $invoicesStart)

    if (-not $invoicesBlock.Contains('const [selectedPayment, setSelectedPayment] = useState<Payment | null>(null);')) {
        $stateAnchor = '  const [selected, setSelected] = useState<Invoice | null>(null);'
        $stateIndex = $invoicesBlock.IndexOf($stateAnchor, [System.StringComparison]::Ordinal)
        if ($stateIndex -lt 0) {
            throw "InvoicesPage selected-state ASCII anchor was not found."
        }
        $stateReplacement = $stateAnchor + [Environment]::NewLine +
            '  const [selectedPayment, setSelectedPayment] = useState<Payment | null>(null);'
        $invoicesBlock = $invoicesBlock.Remove($stateIndex, $stateAnchor.Length)
        $invoicesBlock = $invoicesBlock.Insert($stateIndex, $stateReplacement)
    }

    if (-not $invoicesBlock.Contains('async function loadInvoiceDocument(invoiceId: string)')) {
        $helperAnchor = '  const canPrint = user?.permissions.includes("Invoices.Print") === true;'
        $helperIndex = $invoicesBlock.IndexOf($helperAnchor, [System.StringComparison]::Ordinal)
        if ($helperIndex -lt 0) {
            throw "InvoicesPage canPrint ASCII anchor was not found."
        }
        $helperInsertAt = $helperIndex + $helperAnchor.Length
        $helper = @'
  ...
  async function loadInvoiceDocument(invoiceId: string): Promise<{
    ...
    if (invoice.status === "Issued" && (!payment || payment.status !== "Verified")) {
      throw new Error(
        "پرداخت تأییدشده این فاکتور برای سند چاپی قابل بازیابی نیست.",
      );
    }
...
'@
        $invoicesBlock = $invoicesBlock.Insert($helperInsertAt, $helper)
    }

    # Query-string auto-open must use the same authoritative document path.
    $queryOpenOld = '    if (invoice) setSelected(invoice);'
    if ($invoicesBlock.Contains($queryOpenOld)) {
        $invoicesBlock = $invoicesBlock.Replace(
            $queryOpenOld,
            '    if (invoice) void openInvoicePreview(invoice);')
    }

    # Preview close also clears the authoritative payment paired with the invoice.
    $closeOld = @'
  const closePreview = () => {
    setSelected(null);
...
'@
    if ($invoicesBlock.Contains($closeOld)) {
        $invoicesBlock = $invoicesBlock.Replace($closeOld, $closeNew)
    }
    elseif (-not $invoicesBlock.Contains('setSelectedPayment(null);')) {
        throw "InvoicesPage closePreview ASCII anchor was not found."
    }

    # Every save/print action re-reads the authoritative invoice/payment pair.
    if (-not $invoicesBlock.Contains('const document = await loadInvoiceDocument(invoice.id);')) {
        $desktopBodyAnchor = @'
  ) => {
    if (!isDesktopHost()) {
'@
        $desktopBodyIndex = $invoicesBlock.IndexOf(
            $desktopBodyAnchor,
            $invoicesBlock.IndexOf('const openDesktopDocument = async (', [System.StringComparison]::Ordinal),
            [System.StringComparison]::Ordinal)

        if ($desktopBodyIndex -lt 0) {
            throw "openDesktopDocument body ASCII anchor was not found."
        }
    ...
    if (!isDesktopHost()) {
'@
        $invoicesBlock = $invoicesBlock.Remove($desktopBodyIndex, $desktopBodyAnchor.Length)
        $invoicesBlock = $invoicesBlock.Insert($desktopBodyIndex, $desktopBodyNew)
    }
    ...
    $invoicesBlock = $invoicesBlock.Replace(
        'onClick={() => setSelected(invoice)}><Eye',
        'onClick={() => void openInvoicePreview(invoice)}><Eye')

    # Wire the paired Payment into the exact same preview template.
    $invoiceDetailsCallOld = '<InvoiceDetails invoice={selected} '
    if ($invoicesBlock.Contains($invoiceDetailsCallOld) -and
        -not $invoicesBlock.Contains('<InvoiceDetails invoice={selected} payment={selectedPayment} ')) {
        $invoicesBlock = $invoicesBlock.Replace(
            $invoiceDetailsCallOld,
            '<InvoiceDetails invoice={selected} payment={selectedPayment} ')
    }

    # When leaving preview for edit/print, clear the paired preview payment state.
    $invoicesBlock = $invoicesBlock.Replace(
        'setEditing(selected); setSelected(null);',
        'setEditing(selected); setSelected(null); setSelectedPayment(null);')
    $invoicesBlock = $invoicesBlock.Replace(
        'setPrintTarget(selected); setSelected(null);',
        'setPrintTarget(selected); setSelected(null); setSelectedPayment(null);')

    $operations = $operations.Remove($invoicesStart, $invoicesEnd - $invoicesStart)
    $operations = $operations.Insert($invoicesStart, $invoicesBlock)

    # Patch InvoiceDetails separately so iframe preview shares the same document source.
    ...
    if ($detailsStart -lt 0 -or $detailsEnd -lt 0 -or $detailsEnd -le $detailsStart) {
        throw "Could not isolate InvoiceDetails."
    }

    $detailsBlock = $operations.Substring($detailsStart, $detailsEnd - $detailsStart)
    if (-not $detailsBlock.Contains('  payment,')) {
        $detailsBlock = $detailsBlock.Replace(
            "function InvoiceDetails({`n  invoice,",
            "function InvoiceDetails({`n  invoice,`n  payment,")
        $detailsBlock = $detailsBlock.Replace(
            "  invoice: Invoice;`n",
            "  invoice: Invoice;`n  payment: Payment | null;`n")
    }
    $detailsBlock = $detailsBlock.Replace(
        'srcDoc={buildInvoiceDocumentHtml(invoice)}',
        'srcDoc={buildInvoiceDocumentHtml(invoice, payment)}')

    $operations = $operations.Remove($detailsStart, $detailsEnd - $detailsStart)
    $operations = $operations.Insert($detailsStart, $detailsBlock)
...
'@
    ...
    foreach ($token in @(
        ...
        if (-not $invoiceCheck.Contains($token)) {
            throw "A5 v10.2 invoice-template validation failed: $token"
        }
    }
    ...
    Write-Host ""
    Write-Host "v10.2 README/contract validations passed." -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "SUCCESS: VENDOME A5 invoice data mapping v10.2 installed." -ForegroundColor Green
    ...
    Write-Host "Rolling back every v10.2 invoice change..." -ForegroundColor Yellow
    Restore-All
